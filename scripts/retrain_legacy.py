...
# and does not use TensorFlow Hub modules.
...
def prepare_file_system():
    ...
    # Setup the directory we'll write retrained models to
    if tf.gfile.Exists(FLAGS.output_model):
        tf.gfile.DeleteRecursively(FLAGS.output_model)
    tf.gfile.MakeDirs(FLAGS.output_model)
    if FLAGS.intermediate_store_frequency > 0:
        ensure_dir_exists(FLAGS.intermediate_output_graphs_dir)
    # Setup the directory we'll write bottlenecks to
    if tf.gfile.Exists(FLAGS.bottleneck_dir):
        tf.gfile.DeleteRecursively(FLAGS.bottleneck_dir)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default='',
        help='Path to folder for model containing graph and label'
    )
    parser.add_argument(
        '--output_graph',
        type=str,
